...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:actionButtonTitle style:UIBarButtonItemStyleDone target:self action:@selector(showRoute:)];
...
- (IBAction)showRoute:(id)sender;
{
    ...
    routeViewController.title = @"Route";
    
    [self.navigationController pushViewController:routeViewController animated:YES];
    
    CLLocationCoordinate2D buildingCoords = CLLocationCoordinate2DMake(self.curBuild.coordinate.latitude, self.curBuild.coordinate.longitude);
    
    [routeViewController routeWithEndLocation:buildingCoords withStartLocation:self.mapView.userLocation.coordinate];
}
